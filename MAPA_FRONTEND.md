...
> Generado 2026-09-08 · actualizado 2026-09-21. Si el árbol cambia, este documento miente: regenéralo.
...
| `almacenSalida.ts` | De qué almacén sale un renglón de Venta B2B: pan → almacén de su departamento (`custom_almacen_produccion` gana), lo demás → Bodega Central. Espejo de `sales_invoice.almacen_de_salida` + `constants.almacen_produccion_de` del backend (21-sep). | Sí | Sí (`almacenSalida.test.ts`; 3 mutantes, 3 muertos) |

...
// hay traspaso. El repartidor solo teclea dos columnas por producto:
//
//   REGRESA  lo que trae de vuelta y se revende (pan dulce, vive un día más).
//   SE TIRÓ  el bolillo, o lo que se echa a perder.
//
...
//
// El servidor manda: `guardarRegreso` REEMPLAZA regreso/merma de cada
      ...
      <div className="liq">
        ...
        <section className="liq-tarjeta">
          ...
          {!cargando && renglones.length === 0 && (
            <p className="liq-vacio">Hoy no tienes hoja.</p>
          )}
                  ...
                  {liq.renglones.map(r => (
                    <tr key={r.item_code} className={malos.includes(r.item_code) ? 'liq-fila--mala' : ''}>
                      ...
                      <td className="liq-num">
                        <input
                          className="liq-input" type="text" inputMode="decimal"
                          aria-label={`Se tiró ${r.item_name}`}

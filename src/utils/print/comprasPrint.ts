 ...
 */
export function docToDatosImpresion(doc: any) {
  const totales: any = { subtotal: doc.total || 0, iva: 0, ieps: 0, total: doc.grand_total || 0 };
  let ajuste = 0;
  let descuento = 0;
  (doc.taxes || []).forEach((t: any) => {
  ...
  const filas = (doc.items || []).map((i: any) => {
 ...
 */
export async function imprimirCompraTicket(datos: any) {
  const payload = {
  ...
  try {
    ...
    console.warn('Térmica no disponible, fallback navegador:', (err as any)?.message);
 ...
 */
export function imprimirCompraPDF(datos: any) {
 ...
 * @param factura - No. de factura que consolida las notas (puede ir vacío).
 * @param notas - Filas a sumar ({no_compra, remision, fecha, total}).
 */
export async function imprimirTicketConsolidado(proveedor: string, factura: string, notas: any[], facturadoA = '') {

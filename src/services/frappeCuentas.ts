...
class FrappeCuentasService extends FrappeBase {
  ...
  cambiarNivel(email: string, nivel: string)       { return this._post('cambiar_nivel', { email, nivel }); }
  cambiarPosProfile(email: string, pos_profile: string) { return this._post('cambiar_pos_profile', { email, pos_profile }); }
